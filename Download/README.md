...
When you double-click on the TransGBViewer.exe file, you'll get a warning message: click on the 'Run  anyway' option (Figure 4)
...
# Running TransGBViewer

To run TransGBViewer, double-click on the file in File Explorer, and it should start. If it doesn't, first make sure that all three files are in the same folder (TransGBViewer.exe, TransGBViewer.dll, Newtonsoft.json and TransGBViewer.runtimeconfig.json). If you get an error message such as the one in Figure 6, go to the [.NET download page](https://dotnet.microsoft.com/en-us/download/dotnet/7.0) and select the appropriate ".NET Desktop Runtime". There are a number of options; make sure you choose the desktop version.
