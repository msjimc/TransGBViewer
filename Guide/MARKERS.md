...
Up to this point the image contains only data that has been imported from curated data sources such as the GenBank, the UniProt Consortium and EMBL-EBI's InterPro. User input has been limited to changing the display names and formatting the image. However, the __Markers__ tab allows you to add a range of symbols to aid annotation of the image. These markers are free-floating and can be placed anywhere in the image except in the area reserved for the labels. Once a symbol has been added to the collection of displayed markers, its top left corner is locked to a base pair position in a user-selected data line. This allows the figure to be resized and the order of the transcripts and features to be changed, with the symbol maintaining its position relative to the selected data line (Figure 2).

![Figure 2](images/figuremarker1.jpg)

Figure 2: The __Markers__ tab allows you to add, modify and delete symbols from the image.
...
While a marker's position is set relative to a transcript/feature line, it can be placed anywhere in the image. However, it is best to anchor a marker to a line to which it is referring; otherwise, if the order of the features is changed, its intention may be lost.

The marker's position can be set using either the __X__ and __Y__ numeric controls (blue line in Figure 5) or by selecting the __Mouse click__ option (black line in Figure 5) and clicking on the image. ___Note: When drawing graphs, the X = 0 and Y = 0 position is normally in the bottom left corner; however, when drawing images, this position is in the top left corner___ (red circle in Figure 5). This is done as it simplifies redrawing the image when the picture is resized. Since the X = 0 and Y = 0 position is in the top left corner, increasing the value of the __Y__ numeric control moves the marker towards the bottom of the image.

Generally speaking, it is easier to place a marker in the approximate location by clicking on the image with the  mouse and placing the marker more accurately using the X, Y numeric controls. 

## Displaying a marker

Once the marker's shape, X, Y coordinates and linked data line are selected, the marker will be drawn (Figure 5). The marker will be drawn inside a black box to signify it is only temporary; once the marker has been added to the image's marker collection, the black box will be removed. 

The large green arrow in Figure 5 shows the point from which a marker is drawn: the X, Y coordinates of the box's top left corner are the X, Y coordinates shown in the __X__ and __Y__ numeric controls. It is this point that is anchored to the selected transcript/feature line. This may mean the marker may appear displaced if the width/scale of the image is changed significantly.  
...
The __W__ and __H__ numeric controls (blue line in Figures 6a and 6b) set the symbol’s size, with values ranging from 1 to 25. The __W__ and __H__ values can be adjusted independently of each other. When the marker is resized, it can be seen that the top left corner remains in the same position.
...
Figure 7a: The colour of a marker can be changed by pressing the __Colour__ button (blue line) and selecting the colour using the displayed dialog box (Figure 7b).
