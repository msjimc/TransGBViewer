...
The order of the exons is determined by their presence in the GenBank files. However, there are situations where it is possible to switch the positions of two alternatively spliced exons. For instance, there are two important ketohexokinase (KHK) transcripts: variant a (NM_000221) and variant b (NM_006488) (see files [here](../Data/khk/)). Figure 4 shows the initial display for the KHK transcripts in which the third exon in each transcript is alternatively spliced (blue line in Figure 4) and only present in that transcript. Consequently, the order in the final display is ambiguous and may not reflect the chromosomal order of the exons. 

![Figure 4](images/figure15.jpg)

Figure 4: The position in the display of the third exon of both KHK variants can be switched without affecting the exon order of each transcript.

---

The presence of ambiguously placed exons is detected once the data has been imported and displayed in the checkbox list in the __Adjust alternatively spliced exon order__ panel: in this instance only one ambiguity exists (red line in Figures 5a and 5b). To swap the location of the alternatively spliced exons, tick the box at the side of the relevant checkbox listing and press the __Move__ button (black line in Figure 4). This should then switch the location of the ambiguous exons in the display (green lines in Figures 5a and 5b). 
...
Once the order has been adjusted the exon order is re-examined, and the location of ambiguities is detected, and the checkbox list is repopulated. Since this case is relatively simple, no noticeable change occurs, but this repopulation of the checkbox list means that only one pair of exons can be switched at a time. If you wish to move the exon further to the right, repeat the process until it is in the desired place.  
