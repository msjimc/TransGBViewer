...
Figure 2: Pressing the __Select__ button with __Folder__ option checked prompts you to select a single file.
...
The sequences of each transcript are ues to create a consensus sequence of the gene's transcribed sequences. Initially, a transcript's non-coding sequences are drawn as narrow grey rectangles, while coding sequences are drawn as taller green boxes. Sequence's common to a number of transcripts are drawn above each other allowing common sequences to be identified. If another transcript contains an alternatively spliced exon that is located between two consecutive exons in a transcript a gap is drawn. 
